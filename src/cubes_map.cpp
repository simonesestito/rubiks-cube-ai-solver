 ...
 */
void read_cubes_map(CubesMap& result, const char* filename, size_t from_offset = 0, size_t limit_bytes = 0) {
    std::cout << "Reading cubes map from file " << filename << "..." << std::endl;
    std::ifstream stream(filename, std::ios::binary | std::ios::in);

    // Read 64K block at a time
    char buffer[65536];
    stream.seekg(from_offset);
    ...
    while (read_bytes > 0 && (limit_bytes == 0 || read_file_total < limit_bytes)) {
...
extern "C" void load_cubes_as_list(const char* filename, size_t from_offset, size_t limit_bytes, char* output) {
    std::cout << "Reading cubes map from file " << filename << "..." << std::endl;
    std::ifstream stream(filename, std::ios::binary | std::ios::in);
        ...
        while (read_bytes - cursor >= 25) {
            ...
            for (int face = 0; face < 6; face++) {
                for (int i = 0; i < 3; i++) {
                    for (int j = 0; j < 3; j++) {
                        // For each cell....
                        output[output_cursor++] = static_cast<char>(GET_CUBE(faces[face], i, j));
                    }
                }
            }
            output[output_cursor++] = move;
        }
